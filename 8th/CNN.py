...
def get_batch(image, label, image_W, image_H, batch_size, capacity):
    ...
    image_batch, label_batch = tf.train.batch(input_queue, batch_size=batch_size, num_threads=16, capacity=capacity)
...
def max_pooling_2x2(x, name):
    return tf.nn.avg_pool(x, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name=name)


def inference(images, batch_size, n_classes):
    with tf.variable_scope('conv1') as scope:
        w_conv1 = tf.Variable(weight_variable([5, 1, 1, 16], 1.0), name='weights', dtype=tf.float32)
        b_conv1 = tf.Variable(bias_variable([16]), name='biases', dtype=tf.float32)
    ...
    with tf.variable_scope('conv2') as scope:
        w_conv2 = tf.Variable(weight_variable([5, 1, 16, 32], 0.1), name='weights', dtype=tf.float32)
    ...
    with tf.variable_scope('conv3') as scope:
        w_conv3 = tf.Variable(weight_variable([3, 1, 32, 64], 0.1), name='weights', dtype=tf.float32)
    ...
    h_fc2_dropout = tf.nn.dropout(h_fc1, 0.5)

    with tf.variable_scope('softmax_linear') as scope:
        weights = tf.Variable(weight_variable([32, n_classes], 0.005), name='softmax_linear', dtype=tf.float32)
...
def CNN(acc_sequence, gyr_sequence, label_sequence):
    N_CLASSES = 10
    IMG_W = 28
    IMG_H = 3
    BATCH_SIZE = 300
